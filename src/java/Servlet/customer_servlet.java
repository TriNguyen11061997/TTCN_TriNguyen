    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("HREmployeeID") != null) {
            try {
                ...
                ARCustomersInfo aRCustomersInfo = aRCustomersController.GetObjectByID(id);
                request.setAttribute("Customer", aRCustomersInfo);
                request.getRequestDispatcher("Public/customer.jsp").include(request, response);
            } catch (SQLException ex) {
                Logger.getLogger(customer_servlet.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
